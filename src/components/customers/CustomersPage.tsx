...
function Section({ title, icon }: { title: string; icon: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '6px 0 2px', paddingBottom: 8, borderBottom: '1px solid var(--color-border)' }}>
      ...
      <span style={{ fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--color-text-muted)' }}>{title}</span>
    </div>
  );
}
        ...
        {[
          ...
          <div key={s.label} style={{ padding: '11px 14px', background: s.bg, border: `1px solid ${s.border}`, borderRadius: 10, display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '0.64rem', color: s.col, fontWeight: 600, opacity: 0.75, marginTop: 2, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{s.label}</div>
            </div>
          </div>
        ))}
      </div>
            ...
            <thead>
              <tr>
                {['Code','Name','Type','Terms','Contact','Status',''].map((h, i) => (
                  <th key={h+i} style={{ padding: '10px 12px', textAlign: i >= 5 ? 'center' : 'left', fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', background: 'var(--color-table-head-bg)', color: 'var(--color-table-head-fg)', borderBottom: '2px solid var(--color-border)', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
            ...
            <form onSubmit={handleSubmit} style={{ flex: 1, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {activeTab === 'financial' && (
                <>
                  ...
                  <div style={{ padding: '10px 12px', background: 'rgba(29,78,216,0.05)', borderRadius: 8, border: '1px solid rgba(29,78,216,0.15)', fontSize: '0.78rem', color: '#1d4ed8', lineHeight: 1.6, marginBottom: 4 }}>
                    Link this customer to the AR control account. When invoices are posted, the debit entry will go to the selected account.
                  </div>
                  <div className="form-group" style={{ marginBottom: 0 }}>
                    <label className="form-label">AR Account (Accounts Receivable Control)</label>
                    <select className="input" value={form.ar_account_id ?? ''} onChange={e => setF('ar_account_id', e.target.value || undefined)}>
                      <option value="">(Use company default AR account)</option>
                  ...
                  <div className="form-group" style={{ marginBottom: 0 }}>
                    <label className="form-label">Advance / Deposit Account</label>
