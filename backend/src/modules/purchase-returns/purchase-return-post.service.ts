...
import { PurchaseSupportService } from '../purchases/purchase-support.service.js';
...
interface PurchaseReturnForPost {
  id: string;
  purchase_return_number: string;
  purchase_return_date: string | Date;
...
interface PurchaseReturnLineForPost {
  id: string;
  item_id: string;
  warehouse_id: string;
  quantity: string;
...
export class PurchaseReturnPostService {
  ...
  private async assertDateInOpenPeriod(transaction: TransactionClient, dateText: string) {
    ...
    const result = rows[0];
    if (!result?.can_post) {
      const error = new Error(`Invalid Purchase Return Date: ${result?.reason ?? 'Date cannot be posted.'}`);
  ...
  async post(id: string, userId?: string | null) {
    return prisma.$transaction(async transaction => {
      const returnRows = await transaction.$queryRaw<PurchaseReturnForPost[]>`
        SELECT *
      ...
      if (purchaseReturn.status !== 'Draft') {
        const error = new Error('Only Draft purchase returns can be posted.');
        Object.assign(error, { statusCode: 400 });
        throw error;
      }

      const returnDateText = this.databaseDateText(purchaseReturn.purchase_return_date);
      ...
      const supplierAccount = await this.getAccount(transaction, purchaseReturn.supplier_account_id, 'Supplier Linked Account');
      ...
      const lines = await transaction.$queryRaw<PurchaseReturnLineForPost[]>`
        SELECT *
        FROM purchase_return_lines
        WHERE company_id = CAST(${this.companyId} AS uuid)
          AND purchase_return_id = CAST(${id} AS uuid)
          AND is_active = true
        ORDER BY line_number ASC
      `;
      if (lines.length === 0) {
        const error = new Error('Purchase Return has no items.');
        Object.assign(error, { statusCode: 400 });
        throw error;
      }

      const netAmount = this.money(purchaseReturn.net_amount);
      const inventoryAmount = this.money(purchaseReturn.gross_amount);
      const discountAmount = this.money(purchaseReturn.discount_amount);
      const taxAmount = this.money(purchaseReturn.tax_amount);
      const freightAmount = this.money(purchaseReturn.freight_amount);
      ...
      const voucherLines = [
        {
          account: purchaseReturn.payment_type === 'Cash' ? cashAccount : supplierAccount,
          debit: netAmount,
          credit: new Decimal(0),
          narration: purchaseReturn.payment_type === 'Cash'
            ? `Cash received for ${purchaseReturn.purchase_return_number}`
            : `Supplier payable reduced for ${purchaseReturn.purchase_return_number}`,
        },
        ...(discountAmount.gt(0) && discountAccount ? [{
          account: discountAccount,
          debit: discountAmount,
          credit: new Decimal(0),
          narration: `Reverse purchase discount ${purchaseReturn.purchase_return_number}`,
        }] : []),
        {
          account: inventoryAccount,
          debit: new Decimal(0),
          credit: inventoryAmount,
          narration: `Inventory returned ${purchaseReturn.purchase_return_number}`,
        },
        ...(taxAmount.gt(0) && taxAccount ? [{
          account: taxAccount,
          debit: new Decimal(0),
          credit: taxAmount,
          narration: `Reverse purchase tax ${purchaseReturn.purchase_return_number}`,
        }] : []),
        ...(freightAmount.gt(0) && freightAccount ? [{
          account: freightAccount,
          debit: new Decimal(0),
          credit: freightAmount,
          narration: `Reverse purchase freight ${purchaseReturn.purchase_return_number}`,
        }] : []),
      ];
      ...
      for (const line of lines) {
        await this.postStockLine(transaction, purchaseReturn, line, postedBy);
      }
      ...
      return {
        id,
        purchase_return_number: purchaseReturn.purchase_return_number,
  ...
  private async postStockLine(
    ...
    purchaseReturn: PurchaseReturnForPost,
    line: PurchaseReturnLineForPost,
    userId: string | null,
  ) {
    ...
    const balanceRows = await transaction.$queryRaw<any[]>`
      ...
      WHERE company_id = CAST(${this.companyId} AS uuid)
        ...
        AND (${purchaseReturn.location_id}::uuid IS NULL AND location_id IS NULL OR location_id = ${purchaseReturn.location_id}::uuid)
      FOR UPDATE
    `;
    const existing = balanceRows[0];
    if (!existing) {
      const error = new Error('Cannot post Purchase Return because stock balance was not found.');
      Object.assign(error, { statusCode: 400 });
      throw error;
    }

    const oldQuantity = this.quantity(existing.stock_on_hand ?? 0);
    if (quantity.gt(oldQuantity)) {
      const error = new Error('Cannot post Purchase Return because quantity is greater than Stock On Hand.');
      Object.assign(error, { statusCode: 400 });
      throw error;
    }
    ...
    const newValue = Decimal.max(0, this.money(oldValue.minus(lineCost)));
    ...
    await transaction.$queryRaw`
      ...
      VALUES (
        ...
        ${purchaseReturn.location_id}::uuid,
        CAST(${this.databaseDateText(purchaseReturn.purchase_return_date)} AS date),
        'PURCHASE_RETURN',
        'PURCHASE_RETURN',
        ...
        'WEIGHTED_AVERAGE',
        ${`Purchase Return ${purchaseReturn.purchase_return_number}`},
        true,
        ${userId}::uuid,
        now()
      )
