...
function KpiCard({
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: 12, color: 'var(--color-text-secondary)', letterSpacing: 0 }}>
        {label}
      </span>
...
function SectionHeader({ title }: { title: string }) {
  return (
    <h2 style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text-secondary)', letterSpacing: 0, margin: '0 0 12px' }}>
      {title}
    </h2>
  );
}
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid var(--color-border)' }}>
                  {['Number', 'Type', 'Date', 'Narration', 'Amount', 'Status'].map(h => (
                    <th key={h} style={{ textAlign: 'left', padding: '6px 8px', fontWeight: 600, color: 'var(--color-text-secondary)', fontSize: 11, letterSpacing: 0 }}>
                      {h}
                    </th>
                  ))}
                </tr>
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[
                  ...
                  <div key={item.label} style={{ background: 'var(--color-bg)', borderRadius: 8, padding: '10px 12px' }}>
                    <div style={{ fontSize: 11, color: 'var(--color-text-secondary)', letterSpacing: 0, marginBottom: 4 }}>{item.label}</div>
              ...
              {fiscalYear.currentPeriod && (
                <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 8, padding: '10px 14px' }}>
                  <div style={{ fontSize: 11, color: '#1d4ed8', letterSpacing: 0, marginBottom: 2 }}>Current Period</div>
