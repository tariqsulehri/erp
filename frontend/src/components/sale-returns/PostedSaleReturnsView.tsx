...
import type { CustomerOption, SalePaymentType, WarehouseOption } from '@/components/sales/SaleVoucherTypes';

interface PostedSaleReturnsViewProps {
...
export function PostedSaleReturnsView({
  customers,
  warehouses,
  money,
  generalSettings,
  ...
  const detailQuery = useSaleReturnDetail(selectedId);
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              <h1 style={{ margin: 0, fontSize: '1.05rem', lineHeight: 1.1, color: 'var(--color-heading)' }}>Posted Sale Returns</h1>
        ...
        <div style={{ minHeight: 0, display: 'grid', gridTemplateColumns: selectedId ? 'minmax(0, 1fr) 390px' : '1fr', gap: 8, overflow: 'hidden' }}>
          <div style={{ minHeight: 0, overflow: 'auto', border: '1px solid var(--color-border)', borderRadius: 'var(--radius)' }}>
            <table style={{ width: '100%', minWidth: 980, borderCollapse: 'collapse', fontSize: '0.74rem', tableLayout: 'fixed' }}>
              ...
              <thead>
                <tr>
                  {['Return No.', 'Date', 'Customer', 'Reference', 'Type', 'Warehouse', 'Gross', 'Discount', 'Tax', 'Net Amount', 'View'].map((label, index) => (
                    <th key={label} style={tableHeadStyle(index >= 6 && index <= 9)}>{label}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {listQuery.isLoading && <tr><td colSpan={11} style={{ ...tableCellStyle(), height: 80, textAlign: 'center', color: 'var(--color-text-muted)', fontWeight: 700 }}>Loading Posted Sale Returns...</td></tr>}
                {listQuery.error && <tr><td colSpan={11} style={{ ...tableCellStyle(), height: 80, textAlign: 'center', color: 'var(--color-danger-text)', fontWeight: 700 }}>{friendlyErrorMessage(listQuery.error, 'Unable to load Posted Sale Returns.')}</td></tr>}
                {!listQuery.isLoading && !listQuery.error && rows.length === 0 && <tr><td colSpan={11} style={{ ...tableCellStyle(), height: 90, textAlign: 'center', color: 'var(--color-text-muted)', fontWeight: 700 }}>No posted sale returns found for the selected filters.</td></tr>}
                {rows.map(row => (
                  <tr key={row.id} onClick={() => setSelectedId(row.id)} style={{ cursor: 'pointer', outline: selectedId === row.id ? '2px solid var(--color-primary)' : 'none', outlineOffset: -1 }}>
                    <td style={tableCellStyle()}><strong>{row.sale_return_number}</strong></td>
                    <td style={tableCellStyle()}>{formatDate(row.sale_return_date, generalSettings)}</td>
                    <td style={tableCellStyle()}>{row.customer_code ? `${row.customer_code} - ${row.customer_name}` : row.customer_name}</td>
                    <td style={tableCellStyle()}>{row.reference_number || row.customer_return_number || '—'}</td>
          ...
          {selectedId && (
            <aside style={purchaseDetailPanelStyle}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center', borderBottom: '1px solid var(--color-border)', paddingBottom: 8 }}>
                <strong style={{ fontSize: '0.86rem', color: 'var(--color-heading)' }}>Sale Return Detail</strong>
              ...
              {detailQuery.isLoading && <div style={detailMessageStyle}>Loading sale return detail...</div>}
              {detailQuery.error && <div style={{ ...detailMessageStyle, color: 'var(--color-danger-text)' }}>{friendlyErrorMessage(detailQuery.error, 'Unable to load Sale Return Detail.')}</div>}
              {detailQuery.data && (
                <div style={{ minHeight: 0, display: 'flex', flexDirection: 'column', gap: 8, overflow: 'hidden' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                    {[
                      ['Return No.', detailQuery.data.sale_return_number],
                      ['Date', formatDate(detailQuery.data.sale_return_date, generalSettings)],
                      ['Customer', `${detailQuery.data.customer_code} - ${detailQuery.data.customer_name}`],
                      ['Payment Type', detailQuery.data.payment_type],
                      ['Customer Return No.', detailQuery.data.customer_return_number || '—'],
