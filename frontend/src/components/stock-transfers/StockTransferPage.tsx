...
export default function StockTransferPage() {
  ...
  function addLine() {
    ...
    setLines(current => [
      ...current.filter(line => line.itemId !== selectedItem.value),
      {
        id: Date.now(),
  ...
  return (
    <main style={pageShellStyle}>
      ...
      <section style={lineEntryStyle}>
        ...
        <button className="btn-primary" type="button" onClick={addLine} style={{ height: 28, alignSelf: 'end' }}>Add Line</button>
      </section>
          ...
          <table style={tableStyle}>
            <thead>
              <tr>
                ...
                <HeaderCell width={60}>Delete</HeaderCell>
              </tr>
            </thead>
            <tbody>
              ...
              {lines.map((line, index) => (
                <tr key={line.id}>
                  ...
                  <td style={bodyCellStyle()}><button className="btn-ghost" style={iconButtonStyle} onClick={() => setLines(current => current.filter(item => item.id !== line.id))}><IconTrash size={14} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
const lineEntryStyle: CSSProperties = { border: '1px solid var(--color-border)', background: 'var(--color-surface)', padding: 10, display: 'grid', gridTemplateColumns: 'minmax(360px,1fr) 130px minmax(260px,0.8fr) 100px', gap: 8, flexShrink: 0 };
